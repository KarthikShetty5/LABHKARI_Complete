...
// //     const oid = searchParams ? searchParams.get("orderId") : null;
...
const Page = () => {
  const searchParams = useSearchParams();
  const orderDetails = searchParams ? searchParams.get("orderId") : null;
    ...
    const fetchOrderDetails = async () => {
      ...
      try {
        const response = await fetch(url, {
          ...
          body: JSON.stringify({
            orderId: orderDetails, // Replace with actual orderId
          }),
        });
        const data: { data: Order[] } = await response.json();
        const orderData = data.data[0];
