...
const getWallet = async (req: NextApiRequest, res: NextApiResponse) => {
  ...
  try {
    ...
    if (referralId && !kyc) {
      // User has a referral and has not done KYC
      const recentOrder = await Order.findOne({ userId: userId }).sort({
        createdAt: -1,
      });
      if (recentOrder) {
        ...
        if (batch) {
          ...
          const promo = await Promotional.findOne({ batchId: batchId }).sort({
            applicableDate: -1,
          });
          if (promo) {
            ...
            if (wallet) {
              ...
              wallet.eShopEarning += 0;
              wallet.leaderShipBonus += 0;
              wallet.otherFund += 0;
              wallet.userId = referralId;
