...
const getProductCat = async (req: NextApiRequest, res: NextApiResponse) => {
    ...
    try {
        const products = await Product.find({category}).lean() as { _id: string; customId: string; title: string; description: string; ratings: number; category: string; image: string }[];
