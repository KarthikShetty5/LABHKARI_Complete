...
const getProductId = async (req: NextApiRequest, res: NextApiResponse) => {
    ...
    const customIds = customId.split(',').map((id: string) => id.trim());

    try {
        const results = await Promise.all(customIds.map(async (id: any) => {
            const product = await Product.findOne({ customId: id }).lean();
